...
func main() {
	...
	// Add the cover image
	coverData, err := downloadFile(*coverURL)
	if err != nil {
	...
	for i, line := range urls {
		...
		content, err := extractContent(pageURL, e, tempDir)
		if err != nil {
...
func extractContent(pageURL string, e *epub.Epub, tempDir string) (string, error) {
	// Get the page
	...
	contentDoc.Find("a").Each(func(i int, s *goquery.Selection) {
		...
		if s.Find("img").Length() > 0 {
			...
			imgData, err := downloadFile(fullImgSrc)
			if err != nil {
	...
	contentDoc.Find("img").Each(func(i int, s *goquery.Selection) {
		...
		imgData, err := downloadFile(imgSrc)
		if err != nil {
